...
fn main() {
	...
	while window.is_open() && !window.is_key_down(Key::Escape) {
		...
		let center_world_before = screen_to_world(scx, scy, w, h, zoom, cam_x, cam_y);
		...
		if is_redraw_needed {
			...
			let center_world_after = screen_to_world(scx, scy, w, h, zoom, cam_x, cam_y);
			...
			buffer
				...
				.for_each(|(i, pixel)| {
					let x = (i % w) as float;
					let y = (i / w) as float;
					let (x, y) = screen_to_world(x as float, y as float, w, h, zoom, cam_x, cam_y);
...
fn screen_to_world(
	...
	let aspect_ratio = screen_h as float / screen_w as float;
	let world_x = (x - screen_w as float / 2.0) / (screen_w as float * 0.5) / zoom + cam_x;
	let world_y = (y - screen_h as float / 2.0) / (screen_h as float * 0.5) * aspect_ratio / zoom + cam_y;
	(world_x, world_y)
}
